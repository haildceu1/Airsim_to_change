...
{
public:
    PositionController(Params* params, const IBoardClock* clock = nullptr)  //构造函数，用于初始化对象，冒号后面是用来
    ...
    {   
        ...
        velocity_controller_->initialize(axis, this, state_estimator_);  //为啥用this而不是goal呢？？？？？
    ...
    virtual void update() override
    {
        ...
        pid_-> (measured_position_world[axis_]);
        pid_->update();

        //use this to drive child controller
        velocity_goal_[axis_] = pid_->getOutput() * params_->velocity_pid.max_limit[axis_]; //位置控制器输出 * 速度限制
...
private:
    ...
    std::unique_ptr<PidController<float>> pid_; //独有指针
